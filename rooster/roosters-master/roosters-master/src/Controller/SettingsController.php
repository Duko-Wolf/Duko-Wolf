        ...
        if ($this->isGranted('ROLE_SUPERADMIN')){
            ...
            if ($IconForm->isSubmitted() && $IconForm->isValid()) {
                ...
                if ($favIconFile) {
                    ...
                    foreach (glob($faviconDirectory . '/favicon.*') as $oldFile) {
                        @unlink($oldFile);
                    }
            ...
            if ($LogoForm->isSubmitted() && $LogoForm->isValid()){
                ...
                if ($logoFile) {
                    ...
                    foreach (glob($logoDirectory . '/logo.*') as $oldFile) {
                        @unlink($oldFile);
                    }
            ...
            if ($BackGroundForm->isSubmitted() && $BackGroundForm->isValid()){
                ...
                if ($backgroundFile) {
                    ...
                    $newFilename = 'background.'.$extension;

                    // Verwijder bestaande background.* bestanden
                    foreach (glob($backgroundDirectory . '/background.*') as $oldFile) {
                        @unlink($oldFile);
                    }
            ...
            $backgroundDir = $projectDir . '/public/uploads/background/';
            $iconDir = $projectDir . '/public/uploads/favicon/';
            $logoDir = $projectDir . '/public/uploads/logo/';
            ...
            foreach ($extensions as $ext) {
                if (file_exists($backgroundDir . 'background.' . $ext)) {
                    // This is the web path — without 'public/'
                    $bgFile = '/uploads/background/background.' . $ext;
                    break;
                }
            }
            ...
            foreach ($extensions as $ext) {
                if (file_exists($iconDir . 'fav-icon.' . $ext)) {
                    // This is the web path — without 'public/'
                    $fiFile = '/uploads/favicon/fav-icon.' . $ext;
                    break;
                }
            }
